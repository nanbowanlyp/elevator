...
public:
	...
	virtual void Notify(TNotifyUI& msg) //对消息进行处理
	{
		if (msg.sType == _T("click"))
		{
			...
			if (msg.pSender->GetName() == _T("go"))
			{
				...
				char * init_num = (char*)edit1->GetText().GetData();
				int init_floor = atoi(init_num);
				elevator->setFloor(init_floor);//初始化电梯在第几层
				elevator->setUp();
				elevator->stop();
			/*	CEditUI * edit1 = dynamic_cast <CEditUI*>(m_PaintManager.FindControl(_T("floor_num")));
			...
			else if (msg.pSender->GetName() == _T("search_data"))
			{
			  ::MessageBox(NULL, _T("查表"), _T("查表"), NULL);
			}
